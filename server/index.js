...
if (err) console.log(err)    
 ...
 const imgPoster = req.body.spec.imgPoster;
 const imgSpecs = req.body.spec.imgSpecs;
 ...
 const price = parseInt(req.body.spec.price);
 var id = 0;
 
 
    const insert1 = "insert into crud.mobiles(name, img, price, brand) values(?,?,?,?)";
    db.query(insert1, [phoneName,imgPoster,price,brand])
    ...
    function three(){
    const insert3 = "insert into crud.mob_specs(id, name, price, brand, img, os, ui, dimesion, weight, processor, gpu, screen_size, screen_type, resolution, ram, rom, primary_cam, secondary_cam, battery, network) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    
    // id, name, price, brand, img, os, ui, dimesion, weight, processor, gpu, screen_size, screen_type, resolution, ram, rom, primary_cam, secondary_cam, battery, network
    db.query(insert3, [id,phoneName,price,brand,imgSpecs,os,ui,dimension,weight,processor,gpu,screenSize,screenType,resolution,ram,rom,primaryCam,secondaryCam,battery,network])
    }
  })
